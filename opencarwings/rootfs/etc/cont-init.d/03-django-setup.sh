...
" >> carwings/settings.py
    ...
    OCM_KEY=$(bashio::config 'ocm_api_key' '')
    ITERNIO_KEY=$(bashio::config 'iternio_api_key' '')
...
if os.path.exists(cp_path):
    ...
    if 'if not settings.OPENCHARGEMAP_API_KEY' in content:
        ...
        new_ocm_block = """            if not settings.OPENCHARGEMAP_API_KEY:
                ...
                try:
                    ...
                    logger.error("Failed to parse OCM response: %s", str(e))
                    chargers_resp = []"""
        ...
        new_iternio_block = """            if not settings.ITERNIO_API_KEY:
                ...
                try:
                    ...
                    logger.error("Failed to parse iternio response: %s", str(e))
                    chargers = []"""
